...
function LicenseForm({ walletAddress, contract }) {
  ...
  const uploadFileToSlate = async event => {
    try{
      ...
      const url = `https://uploads.slate.host/api/public/${LicenseTEMPLATE_COLLECTIONID}`;
  ...
  const createLicenseTemplate = async () => {
    try{
      ...
      const res = await contract.methods
        .createLicenseTemplate(cid, window.web3.utils.toWei(price, 'Ether'))
        .send({ from: walletAddress });
      console.log('mintLicenseTemplateNFT', res);
        ...
        <Card.Content>
          <Form>
            <Form.Group widths='equal'>
              <Form.Field>
                <label>Upload License Border (PNG only)</label>
            ...
            {loadingImage
              ? <PlaceholderImage />
              : imageURL && <Image src={imageURL} alt="License Template" fluid />}
